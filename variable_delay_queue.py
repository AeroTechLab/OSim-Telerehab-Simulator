...
localController = LQGPredController( OPERATOR_INERTIA, OPERATOR_DAMPING, OPERATOR_STIFFNESS, NET_TIME_STEP )
...
for timeStepIndex in range( 1, SIM_TIME_STEPS_NUMBER - 1 ):
  ...
  masterFeedback = remoteController.Process( masterOutput, slaveCorrectedOutput, slaveInput )
...
#  if simTime >= masterToSlaveTimesQueue[ 0 ]:
  ...
  slavePredictedOutput = localController.PreProcess( slaveOutput, masterToSlaveDelays[ -1 ] )
  slavePredictedOutputs.append( slavePredictedOutput[ 0 ] )
  
  slaveFeedback = localController.Process( masterOutput, masterFeedback + slaveInput )
  
  slaveCorrectedOutput = localController.PostProcess()
